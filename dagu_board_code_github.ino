...
void loop()
{
  int forwardDist = pingForward(); //send ping forward
  int rightDist = pingRightSide(); //send ping right
  if (forwardDist <= tooClose)
  {
    reverse(); 
    delay(1000); //this should be the time it takes for robot to backup 7"
  }
  if (rightDist <= tooClose)
  {
    leftTurn();
    forward(); 
    delay(1000); //time it takes for robot to move forward 7"
    rightTurn();
  }
  
  int leftDist = pingLeftSide();
  waveCountMax1 = 5;
  
  if (leftDist < 60.96) //24 inches = 60.96 cm
  {
    forward();
    delay(1000); //time it takes robot to move 16" forward
    waveCount1++;
    //where to put compare against waveCountMax1??
  }
  else
  {
    leftTurn(); //90 degree turn left
    forward();
    delay(1000); //time it takes robot to move forward 24 inches
    if (waveCount1 < 3)
    {
      rightTurn();
      waveCountMax2(); //enter switch statement to set waveCountMax2 based upon waveCount1 create function??
    }
    else
    {
      leftTurn();
